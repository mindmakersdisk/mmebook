let livro_cw1 ={

  /* metadados 2 */
  metadados:{
	  i18n:false,
	  formularioRegistro_campos_formatosValidos:['texto','numero','data','url','lista','nome','usuario','senha','usuarioAcessivel','senhaAcessivel'],
	  infografico_estilosValidos:['basicoPaisagem','basicoRetrato','explosaoPaisagem','circuitoPaisagem','cobraPaisagem','cobraPaisagem2', 'cobraRetrato','gPaisagem']},
  
  versao:'0.1.00',
  nomeFantasia: 'Introdução - Alunos em Alfabetização',
  nomeTecnico:'CW1 - Fundamentos em Ciência da Computação',
  resumo:'Módulo adequado à implantação no início do Ensino Fundamental, para alunos em alfabetização. Aborda fundamentos em letramento e pensamento computacional, introduzindo conceitos como sequencia, algoritmo, laço, evento, padrão e outros, de modo lúdico e prático.',
  resumoCapa:'A COMPUTAÇÃO TORNOU A AVIAÇÃO MAIS SEGURA E EFICIENTE. GRAÇAS A SISTEMAS COMPUTADORIZADOS, AVIÕES SÃO MAIS PRECISOS EM VOAR E POUSAR. A SIMULAÇÃO DE VOO TREINA PILOTOS EM AMBIENTES SEGUROS, E O GERENCIAMENTO DE TRÁFEGO AÉREO AUTOMATIZADO MANTÉM OS CÉUS ORGANIZADOS.',
  nomeFantasiaEn: "Introduction - Literacy Students",
  nomeTecnicoEn: "CW1e - Fundamentals of Computer Science",
  resumoEn: "Module suitable for implementation at the beginning of Elementary School, for literacy students. It covers fundamentals of literacy and computational thinking, introducing concepts such as sequence, algorithm, loop, event, pattern, and others, in a playful and practical way.",
  resumoCapaEn: "COMPUTING HAS MADE AVIATION SAFER AND MORE EFFICIENT. THANKS TO COMPUTERIZED SYSTEMS, AIRPLANES ARE MORE PRECISE IN FLYING AND LANDING. FLIGHT SIMULATION TRAINS PILOTS IN SAFE ENVIRONMENTS, AND AUTOMATED AIR TRAFFIC MANAGEMENT KEEPS THE SKIES ORGANIZED.",

  isbn:'978-65-5106-084-7',
  isbnEn: '978-65-5106-082-3',
  autores:['Paulo Alvim, Fernanda Oliveira'],
  editor:'Anderson Borges Gonçalves, Miguel Mendonça de Goffredo',
  local:'',
  ano:'2024',
  url:'pc_cw1_0_1',

  versaoAdaptacao:'',
  nomeAdaptacaoo: '',
  autoresAdaptacao:[],
  local:'',
  escola:'',
  ano:'',
  url:'',
  codigoCid11:'', /* opc */
  descritivoPerfilAluno:'',
  resumoAdaptacao:'',
  
  capa:{
		urlImagemCapa:'',
		urlImagemSegundaCapa: '',
    urlImagemCapaEn:'',
		urlImagemSegundaCapaEn: '',},
  
  formularioRegistro:{
	  titulo:'',
	  urlImagem:'',
	  campos:[
	    {rotulo:'Login Somos:',formato:'usuarioAcessivel'},{rotulo:'Senha:',formato:'senhaAcessivel'}]},
  
  visaogeral:{
		  titulo:'RESUMO DO MÓDULO CW1',
      descricao:'CONHEÇA UM POUCO DO QUE VOCÊ VAI VER NESTE MÓDULO.',
	    licao1:{id:'CW1AULA00RESUMO',
		  infografico:{titulo:'BEM VINDOS AO MÓDULO CW1',
		  tituloSimplificado:'RESUMO',
      estiloPadrao:'basicoPaisagem',
      cenas:[				
        {ordem:1,
        predecessor:0,
        texto:'OLÁ, SEJAM BEM VINDOS AO MÓDULO CW1 DA DISCIPLINA MUNDO COMPUTACIONAL!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res1.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:2,
        predecessor:1,
        texto:'VOCÊS ESTÃO PRESTES A EMBARCAR EM UMA JORNADA EMOCIONANTE, ONDE VAMOS DESCOBRIR JUNTOS COMO FUNCIONAM AS TECNOLOGIAS DIGITAIS QUE USAMOS TODOS OS DIAS!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res2.png',
        textoImpresso:1,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:1,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:3,
        predecessor:2,
        texto:'VOCÊS VÃO APRENDER NA PRÁTICA COMO OS COMPUTADORES E ROBÔS FUNCIONAM.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res5.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:4,
        predecessor:3,
        texto:'NESTE MÓDULO, VAMOS APRENDER SOBRE VÁRIAS COISAS: ENTRE ELAS OS ALGORITMOS, CONDICIONAIS E LAÇOS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res3.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:1,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:5,
        predecessor:4,
        texto:'COM ISSO, PODEMOS PROGRAMAR ROBÔS E COMPUTADORES PARA RESOLVER PROBLEMAS DE UM JEITO INOVADOR E CRIATIVO.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res4.JPG',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:6,
        predecessor:5,
        texto:'E SABE O QUE É MAIS LEGAL? TODO O NOSSO APRENDIZADO SERÁ DE FORMA PRÁTICA E DIVERTIDA!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res6.JPG',
        textoImpresso:1,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:1,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:7,
        predecessor:6,
        texto:'DURANTE ESSA JORNADA, VOCÊS VÃO DESENVOLVER HABILIDADES SUPER IMPORTANTES, COMO ANÁLISE E DECOMPOSIÇÃO DE PROBLEMAS, IDENTIFICAÇÃO DE PADRÕES E PENSAMENTO ALGORÍTMICO.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res7.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:8,
        predecessor:7,
        texto:'ESSAS HABILIDADES SÃO FUNDAMENTAIS PARA ENFRENTAR DESAFIOS, TANTO AGORA QUANTO NO FUTURO!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res8.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},

        {ordem:9,
        predecessor:8,
        texto:'PREPAREM-SE PARA SE DIVERTIR E APRENDER MUITO. VAMOS JUNTOS EXPLORAR O INCRÍVEL MUNDO COMPUTACIONAL!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/resumocw1/res9.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'MM',
        textoDescritivoImagem:'',},
      ]}},
    },
  
  unidade1:{
    titulo:'ALGORITMOS E ROBÔS',
    descricao:'NESTA UNIDADE, VOCÊ SERÁ APRESENTADO AO CONCEITO DE ALGORITMOS E VAI CONHECER O PRIMEIRO ROBÔ QUE VOCÊ VAI PROGRAMAR.',
	  licao1:{id:'CW1AULA01CODE',
		infografico:{titulo:'LIÇÃO 01. DESVENDANDO OS ALGORITMOS',
		tituloSimplificado:'ALGORITMOS',
		estiloPadrao:'basicoPaisagem',
			cenas:[				
				{ordem:1,
				predecessor:0,
        texto:'UM <b>ALGORITMO</b> É UMA INSTRUÇÃO DETALHADA, COM DIFERENTES PASSOS, QUE EXPLICAM COMO RESOLVER UM PROBLEMA OU COMPLETAR UMA TAREFA.',
        urlAudioTexto:'',
        textoSimplificado:'O que é um algoritmo?',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img3.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'ALGORITMOS = INSTRUÇÕES.',
        promptImagem:'Imagem vibrante e alegre de uma lista com diferentes itens marcados como feitos.',
        imagemImpresso:1,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'',},

        {ordem:2,
        predecessor:1,
        texto:'NA PROGRAMAÇÃO, OS PROGRAMADORES ESCREVEM ALGORITMOS QUE DÃO ORDENS AO COMPUTADOR PARA QUE ELE REALIZE UMA DETERMINADA TAREFA.',
        urlAudioTexto:'',
        textoSimplificado:'Os algoritmos e os computadores',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img4.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'ALGORITMOS SERVEM PARA PROGRAMAR COMPUTADORES.',
        promptImagem:'Imagem vibrante e alegre de um personagem infantil sentada de costas, com um laptop à sua frente, com vários balões de linguagens de programação saindo da tela do computador.', 
		    imagemImpresso:1,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'Programador em seu computador',},

        {ordem:3,
        predecessor:2,
        texto:'MAS OS ALGORITMOS NÃO SERVEM SOMENTE PARA OS PROGRAMADORES E COMPUTADORES.',
        urlAudioTexto:'',
        textoSimplificado:'Os algoritmos e as pessoas',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img_5.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'ALGORITMO PARA FAZER UM CORAÇÃO DE PAPEL.',
        promptImagem:'Imagem vibrante e alegre dos passos para se fazer um coração de origami',
        imagemImpresso:0,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'Computador mostrando um algoritmo e papel também mostrando um algoritmo',},

        {ordem:4,
        predecessor:3,
        texto:'UMA RECEITA DE CULINÁRIA É UM ALGORITMO, ASSIM COMO INSTRUÇÕES DE COMO DOBRAR ROUPAS OU MESMO A ROTINA MATINAL, QUE TAMBÉM PODE SER DESCRITA EM UM ALGORITMO!',
        urlAudioTexto:'',
        textoSimplificado:'Uma receita é um algoritmo',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img6.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'RECEITAS TAMBÉM SÃO ALGORITMOS.',
        promptImagem:'Arte digital com seis imagens, uma ao lado da outra, sendo a primeira um prato com 2 fatias de pão. A segunda imagem mostra um pote de maionese ao fundo e uma faca passando maionese em uma fatia de pão. A terceira mostra um pedaço de quijo ao fundo e uma mão colocando uma fatia de queijo no pão que tem maionese. A quarta mostra duas mãos cortando fatias de um tomate. A quinta mostra a fatia de pão em cima do prata, com maionese embaixo, queijo e rodelas de tomate em cima. A última imagem mostra um sanduíche em cima de um prato, feito com pão, maionese, queijo, tomate e outra fatia de pão em cima.', /*opc*/
        imagemImpresso:1,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'',},

        {ordem:5,
        predecessor:4,
        texto:'MAS DEVEMOS FICAR ATENTOS: SE ESTAMOS CRIANDO UM ALGORITMO PARA QUE OUTRA PESSOA EXECUTE, CADA INSTRUÇÃO PRECISA ESTAR MUITO CLARA.',
        urlAudioTexto:'',
        textoSimplificado:'Especificar as instruções',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img9.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'AS INSTRUÇÕES PRECISAM SER CLARAS.',
        promptImagem:'Imagem vibrante e alegre mostrando uma personagem infantil confusa, com olhos arregalados, com uma folha de papel na mão tentando resolver um problema, sem saber o que fazer e vários pontos de interrogação oando acima de sua cabeça.',
        imagemImpresso:1,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'Pessoa confusa com o algoritmo que está tentando seguir',},

        {ordem:6,
        predecessor:5,
        texto:'VEJAMOS O EXEMPLO DO ALGORITMO PARA BEBER UM COPO DE LEITE:',
        urlAudioTexto:'',
        textoSimplificado:'Algoritmo para beber leite',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img10.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
		    promptImagem:'Imagem vibrante e alegre mostrando uma bancada em uma cozinha e um copo cheio de leite em cima da bancada.',
		    imagemImpresso:0,
        imagemFonte:'dall-e 3',		
        textoDescritivoImagem:'',},
  
        {ordem:7,
        predecessor:6,
        texto:'PASSO 1: <br/>&nbsp;&nbsp;<code>PEGUE UM COPO DO ARMÁRIO;</code>',
        urlAudioTexto:'',
        textoSimplificado:'Passo 1',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img11.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'Imagem vibrante e alegre de uma mão de uma pessoa pegando um copo americano de vidro transparente de um armário.',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:8,
        predecessor:7,
        texto:'PASSO 2: <br/>&nbsp;&nbsp;<code>COLOQUE O COPO EM CIMA DA MESA;</code>',
        urlAudioTexto:'',
        textoSimplificado:'Passo 2',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img12.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
		    promptImagem:'Imagem vibrante e alegre de uma mão de um personagem do tipo pessoa colocando um copo de vidro americano transparente e vazio em cima de uma bancada em uma cozinha.',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:9,
        predecessor:8,
        texto:'PASSO 3: <br/>&nbsp;&nbsp;<code>PEGUE O LEITE DA GELADEIRA;</code>',
        urlAudioTexto:'',
        textoSimplificado:'Passo 3',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img13.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
		    promptImagem:'Imagem vibrante e alegre de uma mão de um personagem do tipo pessoa retirando uma caixa de leite de dentro de um geladeira.',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:10,
        predecessor:9,
        texto:'PASSO 4: <br/>&nbsp;&nbsp;<code>COLOQUE O LEITE DENTRO DO COPO QUE ESTÁ EM CIMA DA MESA;</code>',
        urlAudioTexto:'',
        textoSimplificado:'Passo 4',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img14.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
		    promptImagem:'Imagem vibrante e alegre de um copo americano transparente de vidro que está em cima de uma bancada em uma cozinha sendo enchido de leite a partir de uma caixa de leite que uma mão de um personagem do tipo pessoa está segurando.',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:11,
        predecessor:10,
        texto:'PASSO 5: <br/>&nbsp;&nbsp;<code>BEBA O LEITE QUE ESTÁ NO COPO.</code>',
        urlAudioTexto:'',
        textoSimplificado:'Passo 5',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img15.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
		    promptImagem:'Imagem vibrante e alegre de um personagem infantil bebendo leite de um copo americano de vidro transparente cheio de leite. O personagem deve estar bebendo o leite do copo. O copo deve estar na boca do personagem',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:12,
        predecessor:11,
        texto:'POR EXEMPLO: SE O ALGORITMO NÃO DEIXAR CLARO QUE O LEITE VAI "DENTRO" DO COPO, O RESULTADO VAI SER UMA BAGUNÇA!',
        urlAudioTexto:'',
        textoSimplificado:'Instruções não especificadas geram erros',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img_16.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'SEM INSTRUÇÕES CLARAS:',
		    promptImagem:'Uma imagem vibrante e alegre de uma cozinha com uma poça de leite derramado no chão',
        imagemImpresso:0,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'',},
  
        {ordem:13,
        predecessor:12,
        texto:'OS ALGORITMOS NOS AJUDAM A RESOLVER PROBLEMAS GRANDES, PENSANDO EM UMA PARTE DE CADA VEZ.',
        urlAudioTexto:'',
        textoSimplificado:'Algoritmos e resolução de problemas',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/img16.webp',
        textoImpresso:1,
        textoImpressoRecriado:'ALGORITMOS SERVEM PARA DIVIDIR PROBLEMAS GRANDES.',
		    promptImagem:'crie uma imagem colorida de uma menina, em seu quarto, organizando seus brinquedos em diferentes caixas',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
  
        {ordem:14,
        predecessor:13,
        texto:'COM OS ALGORITMOS, VAMOS DEFINIR TAREFAS QUE PESSOAS, COMPUTADORES OU ROBÔS VÃO EXECUTAR, EXATAMENTE COMO DESEJARMOS!',
        urlAudioTexto:'',
        textoSimplificado:'Instruções claras',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i1-algoritmos/imag17.webp',
        textoImpresso:1,
        textoImpressoRecriado:'ALGORITMOS SERVEM PARA MUITA COISA!',
        promptImagem:'crie uma imagem colorida de um garoto negro olhando para seu gráfico de tarefas diárias',
		    imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
			]}},
    
    licao2:{id:'CW1AULA02CODE',
    infografico:{titulo:'LIÇÃO 02. DESCOBRINDO O COMPUTADOR',
    tituloSimplificado:'COMPUTADOR',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
        {ordem:1,
        predecessor:0,
        texto:'COM OS <b>COMPUTADORES</b>, PODEMOS CRIAR VIDEOGAMES, PROGRAMAR ROBÔS E MUITAS OUTRAS COISAS, SEJA PARA DIVERSÃO COM OS AMIGOS OU MESMO PARA RESOLVER PROBLEMAS REAIS E TORNAR O MUNDO UM LUGAR MELHOR PARA TODOS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img2.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'Imagem vibrante e alegre de um personagem infantil programando um computador',
        imagemImpresso:0,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'',},

        {ordem:2,
        predecessor:1,
        texto:'UM COMPUTADOR É UMA MÁQUINA QUE PODEMOS USAR PARA DESENHAR, TOCAR MÚSICAS E ATÉ USAMOS CONVERSAR COM NOSSOS AMIGOS QUE ESTÃO LONGE.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img4.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'UM COMPUTADOR CONSEGUE CONTAR MUITO RÁPIDO, PODEMOS USAR PARA DESENHAR, TOCAR MÚSICAS... E ATÉ PARA CONVERSAR COM NOSSOS AMIGOS QUE ESTÃO LONGE.',
        promptImagem:'criar a imagem de uma videochamada entre três pessoas, todos os jogadores conversando entre si sobre os jogos, mesmo que estejam em países diferentes',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:3,
        predecessor:2,
        texto:'ELE PRECISA DE PESSOAS COMO NÓS PARA DIZER O QUE FAZER, E FAZ TUDO MUITO RÁPIDO!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img6.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'ELE PRECISA DE PESSOAS COMO NÓS PARA DIZER O QUE FAZER.',
        promptImagem:'personagem infantil com um computador de mesa, programando.',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:4,
        predecessor:3,
        texto:'SE PRESTARMOS ATENÇÃO EM UM COMPUTADOR DESSES QUE FICAM EM CASA, SOBRE A MESA, PODEMOS PERCEBER QUE ELE TEM 4 PARTES:',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CSX/DispComp/img_6.png',
        textoImpresso:1,
        textoImpressoRecriado:'ELE TEM PELO MENOS 4 PARTES:',
        promptImagem:'computador desktop em uma mesa moderna e iluminada, mostrando uma CPU, um monitor, um teclado e um mouse',
        imagemImpresso:1,
        imagemFonte:'ilustrador',
        textoDescritivoImagem:'',},

        {ordem:5,
        predecessor:4,
        texto:'A <b>TORRE</b>, OU <b>CPU</b>, ONDE ESTÁ O PROCESSADOR, É COMO SE FOSSE O CÉREBRO DO COMPUTADOR. ELE FAZ CÁLCULOS MUITO RAPIDAMENTE.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CSX/DispComp/img_7.png',
        textoImpresso:1,
        textoImpressoRecriado:'TORRE (OU CPU): É ONDE ESTÁ O PROCESSADOR, QUE É O CÉREBRO DO COMPUTADOR.',
        promptImagem:'a CPU de um computador em ciam de uma mesa',
        imagemImpresso:1,
        imagemFonte:'ilustrador',
        textoDescritivoImagem:'',},

        {ordem:6,
        predecessor:5,
        texto:'O <b>MONITOR</b> É A TELA ONDE PODEMOS VER TODAS AS COISAS QUE O COMPUTADOR MOSTRA PARA NÓS. PODEMOS VER DESENHOS, ASSISTIR A VÍDEOS E JOGAR JOGOS DIVERTIDOS!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CSX/DispComp/img_9.png',
        textoImpresso:1,
        textoImpressoRecriado:'MONITOR: É A TELA ONDE PODEMOS VER TUDO O QUE O COMPUTADOR MOSTRA.',
        promptImagem:'uma mesa clara, bonita e moderna, mostrando uma tela de computador em cima dela',
        imagemImpresso:1,
        imagemFonte:'ilustrador',
        textoDescritivoImagem:'',},

        {ordem:7,
        predecessor:6,
        texto:'O <b>TECLADO</b> É ONDE DIGITAMOS LETRAS, NÚMEROS, SÍMBOLOS E ATÉ MENSAGENS PARA NOSSOS AMIGOS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CSX/DispComp/img_8.png',
        textoImpresso:1,
        textoImpressoRecriado:'TECLADO: É ONDE DIGITAMOS LETRAS, NÚMEROS E SÍMBOLOS.',
        promptImagem:'uma mesa clara, bonita e moderna, mostrando um teclado de computador em cima dela',
        imagemImpresso:1,
        imagemFonte:'ilustrador',
        textoDescritivoImagem:'',},

        {ordem:8,
        predecessor:7,
        texto:'E POR FIM USAMOS O <b>MOUSE</b>, PARA APONTAR E CLICAR NAS COISAS QUE QUEREMOS NO COMPUTADOR. COM ELE, PODEMOS DESENHAR, SELECIONAR COISAS E ATÉ BRINCAR EM JOGOS!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CSX/DispComp/img_10.png',
        textoImpresso:1,
        textoImpressoRecriado:'MOUSE: É O QUE USAMOS PARA APONTAR E CLICAR NAS COISAS QUE APARECEM NA TELA.',
        promptImagem:'uma mesa clara, bonita e moderna, mostrando um mouse de computador em cima dela',
        imagemImpresso:1,
        imagemFonte:'ilustrador',
        textoDescritivoImagem:'',},
 
        {ordem:9,
        predecessor:8,
        texto:'EXISTEM DIFERENTES TIPOS DE COMPUTADORES QUE NOS AJUDAM DE FORMAS DIFERENTES.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img13.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'EXISTEM DIFERENTES TIPOS DE COMPUTADORES.',
        promptImagem:'uma imagem colorida que representa diferentes dispositivos eletrônicos, como smartphones, tablets, computadores',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:10,
        predecessor:9,
        texto:'OS <b>COMPUTADORES DE MESA</b> SÃO USADOS EM ESCOLAS, LOCAIS DE TRABALHO OU EM CASA. ELES NOS AJUDAM A FAZER LIÇÃO DE CASA, JOGAR JOGOS E ASSISTIR A FILMES!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img14.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'COMPUTADORES DE MESA: SÃO OS QUE FICAM EM CASA.',
        promptImagem:'imagem vibrante e alegre de de um computador desktop',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:11,
        predecessor:10,
        texto:'OS <b>LAPTOPS</b> SÃO OS COMPUTADORES DE MESA, MAS SÃO PORTÁTEIS, OU SEJA, PODEMOS LEVÁ-LOS PARA ONDE QUISERMOS. PODEMOS USÁ-LOS PARA FAZER TRABALHOS NA ESCOLA, ASSISTIR A VÍDEOS OU ATÉ MESMO JOGAR JOGUINHOS QUANDO ESTAMOS VIAJANDO!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img15.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'LAPTOPS: SÃO COMO OS COMPUTADORES DE MESA MAS SÃO PORTÁTEIS, OU SEJA, PODEMOS LEVÁ-LOS CONOSCO.',
        promptImagem:'imagem vibrante e alegre que mostre um laptop em cima de uma mesa',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:12,
        predecessor:11,
        texto:'OS <b>TABLETS</b> SÃO COMO OS LAPTOPS, MAS AINDA MAIS LEVES E FÁCEIS DE CARREGAR. PODEMOS USÁ-LOS PARA LER LIVROS, DESENHAR OU ATÉ MESMO APRENDER COISAS NOVAS EM APLICATIVOS DIVERTIDOS!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img16.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'TABLETS: SÃO COMO OS LAPTOPS, PORÉM MAIS LEVES E FÁCEIS DE CARREGAR.',
        promptImagem:'imagem vibrante e alegre que mostre um tablet em cima de uma mesa',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:13,
        predecessor:12,
        texto:'OS <b>CELULARES</b> SÃO COMO PEQUENOS COMPUTADORES QUE CABEM NO BOLSO! PODEMOS USÁ-LOS PARA LIGAR PARA NOSSOS AMIGOS, TIRAR FOTOS E MUITO MAIS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img17.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'CELULARES: SÃO COMO PEQUENOS COMPUTADORES QUE CABEM NO BOLSO!',
        promptImagem:'imagem vibrante e alegre que mostre um celular',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:14,
        predecessor:13,
        texto:'TODOS ESSES TIPOS DE COMPUTADORES NOS AJUDAM A TORNAR NOSSA VIDA MAIS FÁCIL E DIVERTIDA! ELES NOS AJUDAM A APRENDER COISAS NOVAS, A NOS COMUNICAR COM AMIGOS E NOS DIVERTEM COM JOGOS E ANIMAÇÕES.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i2-computador/img18.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'TODOS ESSES TIPOS DE COMPUTADORES TORNAM NOSSA VIDA MAIS FÁCIL E DIVERTIDA!',
        promptImagem:'imagem vibrante e alegre de diferentes tipos de computadores, tablets, celulares e dispositivos eletrônicos',
        imagemImpresso:0,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},
      ]}},

    licao3:{},
      
    licao4:{id:'CW1AULA04CODE',
    infografico:{titulo:'LIÇÃO 04. OS ROBÔS',
    tituloSimplificado:'ROBÔS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
        {ordem:1,
        predecessor:0,
        texto:'UM <b>ROBÔ</b> É UMA MÁQUINA QUE PODE FAZER COISAS DIFERENTES SEM A AJUDA DIRETA DE UMA PESSOA.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/rob4.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'crie uma imagem realista de um drone deixando uma encomenda que está dentro de uma caixa de papelão em frente a uma porta',
        imagemImpresso:0,
        imagemFonte:'Cogna IA',
        textoDescritivoImagem:'',},

        {ordem:2,
        predecessor:1,
        texto:'OS ROBÔS PODEM TER CORPOS FEITOS DE METAL, PLÁSTICO OU OUTROS MATERIAIS E GERALMENTE POSSUEM SENSORES PARA AJUDÁ-LOS A "ENTENDER" O MUNDO AO SEU REDOR.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/imag5.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'ROBÔS FAZEM MUITAS COISAS.',
        promptImagem:'Robô engraçado 3D explorando outros planetas',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:3,
        predecessor:2,
        texto:'EXISTEM MUITOS TIPOS DIFERENTES DE ROBÔS. ALGUNS TÊM BRAÇOS E PERNAS COMO OS HUMANOS, ENQUANTO OUTROS TÊM RODAS OU ATÉ MESMO ASAS!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/imag6.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'ROBÔS PODEM SER COMO GENTE OU BEM DIFERENTES.',
        promptImagem:'Robô humanóide 3D engraçado andando na rua',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:4,
        predecessor:3,
        texto:'ALGUNS ROBÔS SÃO USADOS EM FÁBRICAS PARA AJUDAR A CONSTRUIR CARROS, BRINQUEDOS E OUTRAS COISAS QUE USAMOS TODOS OS DIAS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/rob7.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'crie uma imagem realista de uma fábrica com a linha de produção completamente robotizada',
        imagemImpresso:0,
        imagemFonte:'Cogna IA',
        textoDescritivoImagem:'',},

        {ordem:5,
        predecessor:4,
        texto:'OUTROS ROBÔS SÃO USADOS EM HOSPITAIS PARA AJUDAR OS MÉDICOS A FAZEREM CIRURGIAS COM PRECISÃO OU PARA CUIDAR DAS PESSOAS QUANDO ESTÃO DOENTES.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/rob8.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'crie uma imagem realista de uma sala de cirurgia, com um médico utilizando um robô como assistente durante um procedimento',
        imagemImpresso:0,
        imagemFonte:'Cogna IA',
        textoDescritivoImagem:'',},

        {ordem:6,
        predecessor:5,
        texto:'ALÉM DISSO, EXISTEM ROBÔS QUE PODEMOS PROGRAMAR PARA FAZER TAREFAS ESPECÍFICAS. ISSO SIGNIFICA QUE PODEMOS DAR INSTRUÇÕES A ELES E ELES AS SEGUIRÃO.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/rob9.png',
        textoImpresso:1,
        textoImpressoRecriado:'ROBÔS TÊM UM COMPUTADOR COMO CÉREBRO.',
        promptImagem:'gere uma imagem de um robô com uma bandeja na mão, levando um medicamento para uma pessoa que está deitada no sofá',
        imagemImpresso:1,
        imagemFonte:'Cogna IA',
        textoDescritivoImagem:'',},

        {ordem:7,
        predecessor:6,
        texto:'OS ROBÔS SÃO PROGRAMADOS USANDO CÓDIGOS E COMANDOS COM INSTRUÇÕES. É COMO ENSINAR UM ROBÔ A FAZER ALGO.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/img11.webp',
        textoImpresso:1,
        textoImpressoRecriado:'ROBÔS PRECISAM SER PROGRAMADOS PARA AGIR.',
        promptImagem:'Robô engraçado 3D ao lado de um computador, ouvindo o que o computador tem a dizer',
        imagemImpresso:1,
        imagemFonte:'dall-e',
        textoDescritivoImagem:'',},

        {ordem:8,
        predecessor:7,
        texto:'É IMPORTANTE LEMBRAR QUE OS ROBÔS SÃO APENAS MÁQUINAS E NÃO TÊM SENTIMENTOS COMO OS SERES HUMANOS. ELES SEGUEM NOSSAS INSTRUÇÕES, MAS NÃO PODEM PENSAR OU SENTIR COMO NÓS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.cloud.google.com/mindmakers/Computational%20World/CW1/i3-robos/img13.webp',
        textoImpresso:1,
        textoImpressoRecriado:'ROBÔS NÃO PENSAM, SÓ OBEDECEM.',
        promptImagem:'Robô engraçado 3D olhando para um computador, como se estivesse esperando que ele dissesse algo',
        imagemImpresso:1,
        imagemFonte:'dall-e',
        textoDescritivoImagem:'',},

        {ordem:9,
        predecessor:8,
        texto:'AGORA, QUE TAL EXPLORARMOS MAIS SOBRE OS ROBÔS E SUAS INCRÍVEIS CAPACIDADES?',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i3-robos/rob15.png',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'gere uma imagem de robôs diferentes',
        imagemImpresso:0,
        imagemFonte:'Cogna IA',
        textoDescritivoImagem:'',},
        ]}},

      licao5:{},
    
      licao6:{id:'CW1AULA06CODE',
      infografico:{titulo:'LIÇÃO 06. LETRÓPOLIS: A CIDADE DAS VOGAIS',
      tituloSimplificado:'LETRÓPOLIS',
      estiloPadrao:'basicoPaisagem',
        cenas:[				
        {ordem:1,
        predecessor:0,
        texto:'HOJE TEMOS UMA CIDADE CURIOSA PARA VISITAR: <b>LETRÓPOLIS</b>, UMA CIDADE ONDE SÓ VIVEM ROBÔS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img2.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'HOJE VAMOS CONHECER UMA CIDADE MUITO DIFERENTE: O NOME DELA É LETRÓPOLIS! ',
        promptImagem:'uma ilha nevada colorida e futurista',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:2,
        predecessor:1,
        texto:'EM LETRÓPOLIS, OS ROBÔS AMAM AS LETRAS, ESPECIALMENTE AS VOGAIS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img3.webp',
        textoImpresso:1,
        textoImpressoRecriado:'E COMO O NOME SUGERE, OS ROBÔS DE LETRÓPOLIS AMAM LETRAS, PRINCIPALMENTE AS VOGAIS!',
        promptImagem:'crie uma imagem colorida de dois robôs engraçados em 3D conversando entre si em uma rua futurista e com neve. ambos têm balões de fala. Na direita você pode ver a letra “E” e na esquerda você pode ver a letra “A”',
        imagemImpresso:1,
        imagemFonte:'dall-e 3',
        textoDescritivoImagem:'',},

        {ordem:3,
        predecessor:2,
        texto:'NA PEQUENA ILHA GELADA DE LETRÓPOLIS, OS ROBÔS FAZEM DE TUDO, SÃO GUARDAS, MOTORISTAS, FAZENDEIROS. ELES DÃO UM JEITO EM TUDO! SÓ NÃO CONSEGUEM AINDA ESCREVER, DESENHAR E PINTAR.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img4.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'NA ILHA GELADA DE LETRÓPOLIS, OS ROBÔS FAZEM DE TUDO PARA MANTER A ENERGIA LÁ NO ALTO.',
        promptImagem:'uma rua colorida e futurista em uma ilha nevada',
        imagemImpresso:0,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},
        
        {ordem:4,
        predecessor:3,
        texto:'E HOJE OS HABITANTES DE LETRÓPOLIS PRECISAM DE AJUDA: ESTÁ UMA BAITA CONFUSÃO POR LÁ!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img5.jpeg',
        textoImpresso:1,
        textoImpressoRecriado:'OS ROBÔS DE LETRÓPOLIS ESTÃO COM UM BAITA PROBLEMA PARA TRANSITAR NA CIDADE.',
        promptImagem:'robôs engraçados andando em uma rua futurística com neve pareciam muito confusos, pois não sabiam como chegar a algum lugar',
        imagemImpresso:1,
        imagemFonte:'ideogram',
        textoDescritivoImagem:'',},

        {ordem:5,
        predecessor:4,
        texto:'AS RUAS DA CIDADE NÃO TÊM PLACAS, OS ROBÔS NÃO SABEM AO CERTO PARA ONDE ESTÃO INDO.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img6.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'AS RUAS DA CIDADE NÃO TÊM PLACAS, ENTÃO OS ROBÔS FICAM PERDIDOS!',
        promptImagem:'uma rua colorida e futurista de uma ilha nevada com robôs passando, como população',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:6,
        predecessor:5,
        texto:'MAS OLHA QUE IDEIA GENIAL: QUE TAL PINTAR AS VOGAIS NAS RUAS, JÁ QUE EM LETRÓPOLIS TODOS ADORAM AS VOGAIS?',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img7.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'E VOCÊ FOI CONVOCADO PARA AJUDAR A PINTAR AS VOGAIS NAS RUAS, USANDO PROGRAMAÇÃO.',
        promptImagem:'uma rua colorida e futurista de uma ilha nevada com um robô carregando um balde com tinta e um pincel',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:7,
        predecessor:6,
        texto:'COM ISSO, OS ROBÔS VÃO PODER DIZER PARA QUAL RUA QUEREM IR SOMENTE FALANDO A LETRA QUE REPRESENTA A RUA. SERÁ O FIM DA CONFUSÃO EM LETRÓPOLIS.',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/let7.jpg',
        textoImpresso:0,
        textoImpressoRecriado:'',
        promptImagem:'',
        imagemImpresso:0,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},

        {ordem:8,
        predecessor:7,
        texto:'SERÁ QUE CONSEGUIMOS AJUDAR OS ROBÔS DESSA CIDADE USANDO PROGRAMAÇÃO? MÃOS À OBRA!',
        urlAudioTexto:'',
        textoSimplificado:'',
        urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img10.jpg',
        textoImpresso:1,
        textoImpressoRecriado:'VAMOS AJUDAR? MÃOS À OBRA!',
        promptImagem:'uma ilha nevada colorida e futurista, vista de longe',
        imagemImpresso:1,
        imagemFonte:'leonardo',
        textoDescritivoImagem:'',},
      ]}},

    licao7:{},
    
  },

  unidade2:{
    titulo:'LAÇOS E EVENTOS',
    descricao:'NESTA UNIDADE, VOCÊ APRENDERÁ A USAR OS LAÇOS PARA REPETIR COMANDOS E VERÁ COMO OS EVENTOS ATUAM NA PROGRAMAÇÃO.',
    licao8:{id:'CW1AULA08CODE',
    infografico:{titulo:'LIÇÃO 08. CIRCUITOS EM PAPEL: UMA JORNADA ELETRIZANTE',
    tituloSimplificado:'CIRCUITOS EM PAPEL',
    estiloPadrao:'basicoPaisagem',
      cenas:[
      {ordem:1,
      predecessor:0,
      texto:'A <b>ELETRICIDADE</b> É A ENERGIA QUE FAZ TODAS AS COISAS FUNCIONAREM. ELA FLUI ATRAVÉS DE FIOS E NOS AJUDA A ACENDER LUZES, LIGAR A TV E FAZER MUITAS OUTRAS COISAS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img2.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'ELETRICIDADE É A ENERGIA QUE FAZ TODAS AS COISAS FUNCIONAREM.',
      promptImagem:'Uma imagem em close de um relâmpago vibrante formando um arco em um céu escuro',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'UM <b>CIRCUITO</b> É COMO SE FOSSE UM CAMINHO A SER PERCORRIDO, QUE DÁ UMA VOLTA COMPLETA E SE FECHA, COMO SE FOSSE UMA PISTA DE CARROS DE CORRIDA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img5.jpeg',
      textoImpresso:0,
      textoImpressoRecriado:'UM CIRCUITO É COMO SE FOSSE UM CAMINHO A SER PERCORRIDO, QUE DÁ UMA VOLTA COMPLETA E SE FECHA.',
      promptImagem:'Uma vista aérea de um intenso circuito de corridas de automóveis, com múltiplas curvas e curvas fechadas',
      imagemImpresso:0,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'UM <b>CIRCUITO ELÉTRICO</b> É COMO UM CAMINHO PARA A ELETRICIDADE, MAS EM VEZ DE UMA PISTA, USAMOS FIOS OU FITAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img6.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'UM CIRCUITO ELÉTRICO É COMO UM CAMINHO PARA A ELETRICIDADE, ONDE USAMOS FIOS OU FITAS.',
      promptImagem:'um cabo elétrico visto por dentro, com energia percorrendo-o',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'PARA MONTAR UM <b>CIRCUITO EM PAPEL</b>, PRECISAMOS DE UMA <b>FITA DE COBRE</b>, QUE VAI SERVIR DE CAMINHO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img8.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'PARA MONTAR UM CIRCUITO EM PAPEL, PRECISAMOS DE UMA FITA DE COBRE, QUE VAI SERVIR DE CAMINHO.',
      promptImagem:'fita elétrica de cobre',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'PRECISAMOS TAMBÉM DE UMA <b>BATERIA</b>. ELAS NOS DÃO A ELETRICIDADE DE QUE PRECISAMOS PARA FAZER AS COISAS FUNCIONAREM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img9.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'PRECISAMOS TAMBÉM DE UMA BATERIA. ELAS NOS DÃO A ELETRICIDADE.',
      promptImagem:'lithium battery',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'QUANDO OLHAMOS PARA A BATERIA, VEMOS DOIS LADOS E UM DESSES LADOS TEM UM SÍMBOLO DE MAIS (+). ESTE SÍMBOLO VAI NOS AJUDAR A COLOCAR TODAS AS PARTES DO CIRCUITO NO LUGAR CERTO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img10.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'E PRECISAMOS TAMBÉM DE UM <b>LED</b>. ELE É COMO SE FOSSE UMA LÂMPADA, QUE VAI ACENDER NO NOSSO CIRCUITO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img14.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'E PRECISAMOS DE UM LED, QUE VAI ACENDER NO CIRCUITO.',
      promptImagem:'LED',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'DEPOIS QUE TEMOS TODOS OS MATERIAIS, PODEMOS MONTAR UM CIRCUITO E VER A LUZ SE ACENDER!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img11.jpeg',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'',
      imagemImpresso:0,
      imagemFonte:'MM',
      textoDescritivoImagem:'',},

      {ordem:9,
      predecessor:8,
      texto:'MAS ATENÇÃO: NUNCA DEVEMOS MEXER COM ELETRICIDADE SEM A AJUDA OU SUPERVISÃO DE UM ADULTO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img18.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'LEMBRE-SE DE NUNCA MEXER COM ELETRICIDADE SEM A AJUDA OU SUPERVISÃO DE UM ADULTO.',
      promptImagem:'Uma foto cativante de uma criança curiosa montando diligentemente um circuito elétrico usando fita de cobre e uma variedade de LEDs coloridos. A criança está absorta em seu trabalho, exposto em uma bancada de madeira. No fundo, uma figura adulta, provavelmente um mentor ou pai, observa com um sorriso, pronto para oferecer orientação ou assistência.',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:10,
      predecessor:9,
      texto:'E NUNCA DEVEMOS COLOCAR OBJETOS NA TOMADA OU BRINCAR COM FIOS ELÉTRICOS. DEVEMOS SEMPRE ESTAR EM SEGURANÇA PARA EXPLORAR O MUNDO DA ELETRICIDADE!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i5-circuitos/img19.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'E NUNCA COLOQUE OBJETOS NA TOMADA OU BRINQUE COM FIOS ELÉTRICOS.',
      promptImagem:'uma criança segurando o fio de um rádio, que está conectado a uma tomada, puxando-o',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},
      ]}},
	 
    licao9:{id:'CW1AULA09CODE',
    infografico:{titulo:'LIÇÃO 09. VAMOS REPETIR! APRENDENDO SOBRE LAÇOS',
    tituloSimplificado:'LAÇOS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'SABE QUANDO FAZEMOS ALGO VÁRIAS VEZES, COMO PULAR CORDA OU BATER PALMAS? OS COMPUTADORES TAMBÉM PODEM FAZER ISSO, E PARA ISSO ELES USAM OS <b>LAÇOS!</b>',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i6-lacos/img1.webp',
      textoImpresso:1,
      textoImpressoRecriado:'OS LAÇOS SERVEM PARA DIZER, A PESSOAS OU COMPUTADORES, QUE REPITAM ALGO VÁRIAS VEZES.',
      promptImagem:'criança pulando corda, desenho animado, cinematic',
      imagemImpresso:1,
      imagemFonte:'dall-e',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'OS COMPUTADORES CONSEGUEM LEMBRAR DE COISAS MUITO BEM. QUANDO QUEREMOS QUE UM COMPUTADOR FAÇA ALGO VÁRIAS VEZES, EM VEZ DE DIZER PARA ELE FAZER A MESMA COISA DE NOVO E DE NOVO, PODEMOS USAR OS LAÇOS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i6-lacos/img4.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'DÊ UM COMANDO PARA O COMPUTADOR OU ROBÔ REPETIR ALGO E ELE VAI OBEDECER.',
      promptImagem:'computador desktop com o símbolo do infinito na tela, desenho animado',
      imagemImpresso:1,
      imagemFonte:'autoral',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'VAMOS IMAGINAR QUE QUEREMOS DESENHAR MUITAS CARINHAS SORRIDENTES NA TELA DO COMPUTADOR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i6-lacos/img4.webp',
      textoImpresso:1,
      textoImpressoRecriado:'POR EXEMPLO, SE VOCÊ QUISER FAZER O MESMO DESENHO VÁRIAS VEZES, COMO A CARINHA FELIZ...',
      promptImagem:'crie uma imagem colorida de uma tela de computador mostrando um rosto sorridente amarelo',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'EM VEZ DE DIZER "DESENHE UMA CARINHA SORRIDENTE" DE NOVO E DE NOVO, PODEMOS USAR UM LAÇO E DIZER: "DESENHE UMA CARINHA SORRIDENTE 5 VEZES!" O COMPUTADOR VAI FAZER ISSO RAPIDINHO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CS1/La%C3%A7os/img5.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'... BASTA DAR O COMANDO REPITA EM VOLTA DA TAREFA.',
      promptImagem:'comando repita',
      imagemImpresso:1,
      imagemFonte:'autoral',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'OS LAÇOS AJUDAM O COMPUTADOR A SER MAIS RÁPIDO E EFICIENTE. NO FUTURO, QUANDO OS LAÇOS VÃO NOS AJUDAR A FAZER COISAS INCRÍVEIS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i6-lacos/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'O COMPUTADOR VAI REPETIR A TAREFA MUITAS VEZES.',
      promptImagem:'desenho animado de um computador com um balão de fala vazio em cima',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

    ]}},

    licao10:{},

    licao11:{id:'CW1AULA11CODE',
    infografico:{titulo:'LIÇÃO 11. DESCOBRINDO OS EVENTOS NA PROGRAMAÇÃO',
    tituloSimplificado:'EVENTOS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'UM <b>EVENTO</b> É ALGO QUE ACONTECE EM UM DETERMINADO MOMENTO E LUGAR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img1.webp',
      textoImpresso:0,
      textoImpressoRecriado:'UM EVENTO É ALGO QUE ACONTECE EM ALGUM MOMENTO.',
      promptImagem:'crie uma imagem colorida, estilo quadrihos, de um despertador tocando',
      imagemImpresso:0,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'O NASCER DO SOL, O ANOITECER E OUTROS EXMPLOS COMO ESSE SÃO EVENTOS QUE OCORREM TODOS OS DIAS EM NOSSAS VIDAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img2.webp',
      textoImpresso:1,
      textoImpressoRecriado:'EVENTOS PODEM ACONTECER TODO DIA: "O SOL NASCEU".',
      promptImagem:'crie uma imagem colorida, estilo história em quadrinhos, do sol nascendo pela manhã',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'UM ARCO-ÍRIS, QUANDO OCORRE, É UM EVENTO INESPERADO QUE NOS FAZ REAGIR: "NOSSA, QUE LINDO!"',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img3.webp',
      textoImpresso:1,
      textoImpressoRecriado:'OU QUANDO MENOS ESPERAMOS: “OLHA, UM ARCO-ÍRIS!”',
      promptImagem:'arte digital de um arco-iris',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'PROGRAMAS DE COMPUTADORES TAMBÉM ENTENDEM EVENTOS. ELES SÃO PROGRAMADOS PARA REALIZAR AÇÕES QUANDO CERTOS EVENTOS ACONTECEM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img4.webp',
      textoImpresso:1,
      textoImpressoRecriado:'COMPUTADORES TÊM EVENTOS PROGRAMADOS.',
      promptImagem:'arte digital de um computador',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'POR EXEMPLO, QUANDO TOCAMOS A TELA DE UM TABLET, ISSO É UM <b>EVENTO DE TOQUE</b>.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img7.webp',
      textoImpresso:1,
      textoImpressoRecriado:'TOCAR NA TELA DO CELULAR OU DO  TABLET É UM EVENTO.',
      promptImagem:'uma pessoa usando um tablet, arte digital',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'QUANDO APERTAMOS UMA TECLA EM UM TECLADO, ISSO É UM <b>EVENTO DE TECLADO</b>.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img8.webp',
      textoImpresso:1,
      textoImpressoRecriado:'APERTAR UMA TECLA NO TECLADO É UM EVENTO.',
      promptImagem:'arte digital de um dedo apertando uma tecla em um teclado de computador',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'OS COMPUTADORES ESTÃO SEMPRE PRONTOS PARA DETECTAR ESSES EVENTOS E TOMAR AÇÕES COM BASE NELES.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img10.webp',
      textoImpresso:1,
      textoImpressoRecriado:'JOGAR VIDEOGAMES NO JOYSTICK CRIA EVENTOS.',
      promptImagem:'um computador desktop com código flutuando, arte digital',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'QUANDO PRESSIONAMOS UM BOTÃO PARA PULAR EM UM JOGO, O PERSONAGEM PULA!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img9.webp',
      textoImpresso:1,
      textoImpressoRecriado:'EVENTOS FAZEM OS PROGRAMAS DE COMPUTADOR REAGIREM.',
      promptImagem:'arte digital de uma criança jogando videogame',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:9,
      predecessor:8,
      texto:'ISSO ACONTECE PORQUE O COMPUTADOR ENTENDEU O EVENTO DO BOTÃO SENDO PRESSIONADO E TOMOU A AÇÃO DE FAZER O PERSONAGEM PULAR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img11.webp',
      textoImpresso:1,
      textoImpressoRecriado:'NO VIDEOGAME, UM EVENTO DO JOYSTICK CAUSA UMA REAÇÃO PROGRAMA DO JOGO, QUE FAZ O PERSONAGEM PULAR.',
      promptImagem:'arte digital de um computador desktop com código flutuando',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:10,
      predecessor:9,
      texto:'OS EVENTOS ESTÃO POR TODA PARTE, DESDE AS COISAS QUE FAZEMOS QUANDO ACORDAMOS ATÉ AS AÇÕES QUE TOMAMOS COM A AJUDA DE DISPOSITIVOS ELETRÔNICOS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i7-eventos/img11.png',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'gere uma imagem, em estilo revista em quadrinhos, colorida, de um computador desktop, um tablet, um celular, um videogame e uma TV',
      imagemImpresso:0,
      imagemFonte:'Cogna IA',
      textoDescritivoImagem:'',},
    ]}},

    licao12:{},
	  
    licao13:{id:'CW1AULA13CODE',
    infografico:{titulo:'LIÇÃO 13. BEM VINDOS A NUMERÓPOLIS!',
    tituloSimplificado:'NUMERÓPOLIS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'HOJE VAMOS CONHECER UMA OUTRA CIDADE: O NOME DELA É <b>NUMERÓPOLIS</b>.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img1.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'ESTA É NUMERÓPOLIS. ',
      promptImagem:'cidade futurista colorida vista de cima',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'NUMERÓPOLIS TAMBÉM É UMA CIDADE SUPER ROBOTIZADA, ONDE SOMENTE VIVEM ROBÔS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img3.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'NUMERÓPOLIS É UMA CIDADE SUPER ROBOTIZADA, ONDE SÓ VIVEM ROBÔS.',
      promptImagem:'arte digital de uma cidade colorida e futurista, vista de cima',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'E, COMO O PRÓPRIO NOME JÁ DIZ, OS ROBÔS DAQUI AMAM OS NÚMEROS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img4.webp',
      textoImpresso:1,
      textoImpressoRecriado:'EM NUMERÓPOLIS, OS ROBÔS AMAM OS NÚMEROS!',
      promptImagem:'crie uma imagem colorida de uma rua futurista onde você pode ver dois robôs 3D engraçados conversando entre si. Ambos têm balões de fala flutuando acima de suas cabeças. No balão de fala direito você pode ver o número "3" e no balão de fala esquerdo você pode ver o número "7"',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'OS ROBÔS DE NUMERÓPOLIS ESTÃO COM UM GRANDE PROBLEMA E PRECISAM DE AJUDA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'MAS OS ROBÔS DE NUMERÓPOLIS ESTÃO COM UM BAITA PROBLEMA PARA TRANSITAR NA CIDADE.',
      promptImagem:'arte digital de uma rua colorida e futurista, mostrando um robô engraçado coçando a cabeça',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'AS RUAS DA CIDADE NÃO TÊM PLACAS, OS ROBÔS NÃO SABEM AO CERTO PARA ONDE ESTÃO INDO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img6.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'AS RUAS DA CIDADE NÃO TÊM PLACAS, ENTÃO OS ROBÔS FICAM PERDIDOS!',
      promptImagem:'arte digital de uma rua colorida e futurista, mostrando um robô engraçado andando confuso pela rua',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'JÁ VIMOS ESSE PROBLEMA ANTES, NÃO É?',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/num6.jpeg',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'Arte digital de uma rua colorida e futurista com um robô 3D engraçado andando confuso',
      imagemImpresso:0,
      imagemFonte:'dall-e',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'PODEMOS AJUDAR, PROGRAMANDO ROBÔS PARA PINTAR UM NÚMERO DIFERENTE, EM CADA RUA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img8.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'PODEMOS AJUDAR, PROGRAMANDO ROBÔS PARA PINTAR UM NÚMERO DIFERENTE, EM CADA RUA.',
      promptImagem:'personagem infantil com um computador de mesa, programando',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'DANDO NÚMEROS ÀS RUAS, VAMOS ACABAR COM A CONFUSÃO EM NUMERÓPOLIS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i8-numeropolis/img9.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'DANDO NÚMEROS ÀS RUAS, VAMOS ACABAR COM A CONFUSÃO EM NUMERÓPOLIS!',
      promptImagem:'Robô engraçado 3D acenando com a mão',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},
    ]}},

    licao14:{},
  },
   
  unidade3:{
    titulo:'CONDICIONAIS E DEPURAÇÃO',
    descricao:'NESTA UNIDADE, VOCÊ APRENDERÁ A USAR CONDICIONAIS E SE TORNARÁ UM VERDADEIRO DETETIVE, PARA ENCONTRAR BUGS NOS PROGRAMAS.',
    licao15:{id:'CW1AULA15CODE',
    infografico:{titulo:'LIÇÃO 15. EXPLORANDO CAMINHOS COM AS CONDICIONAIS',
    tituloSimplificado:'CONDICIONAIS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'VAMOS IMAGINAR QUE VOCÊ QUER COMER PIPOCA ENQUANTO VÊ UM FILME.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img3.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'IMAGINE QUE VOCÊ QUER VER UM FILME E COMER PIPOCA.',
      promptImagem:'pintura de uma televisão a cores numa parede e uma mesa de café em frente. em cima da mesa é possível ver um bowl cheio de pipoca',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'MAS A SUA MÃE DIZ QUE VOCÊ SÓ PODE VER O FILME E COMER PIPOCA SE O DEVER DE CASA ESTIVER PRONTO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img3.webp',
      textoImpresso:1,
      textoImpressoRecriado:'MAS PARA ISSO, PRECISA TER FEITO O DEVER DE CASA.',
      promptImagem:'pintura de uma mesa cheia de livros abertos, canetas, lápis e cadernos',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'ELA ESTABELECEU UMA CONDICIONAL</b>: <code><b>SE</b> VOCÊ FEZ O DEVER DE CASA <br/>&nbsp;&nbsp;<b>ENTÃO</b> VOCÊ PODE VER O FILME E COMER PIPOCA</code>',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'SE FEZ O DEVER DE CASA, ENTÃO VEJA FILME E COMA PIPOCA.',
      promptImagem:'pintura de uma personagem infantil sentada em um sofá, embrulhada em um cobertor e segurando um bowl cheio de pipoca',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'ESSA CONDICIONAL MOSTRA EXATAMENTE O QUE É PRECISO PARA VER O FILME E COMER PIPOCA. É COMO UMA REGRA QUE NOS GUIA!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img6.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'A ESCOLHA DEPENDE DA CONDIÇÃO.',
      promptImagem:'pintura de uma pessoa parada em frente a dois caminhos diferentes na floresta',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'OS COMPUTADORES PODEM SEGUIR INSTRUÇÕES COM BASE EM CONDIÇÕES QUE PROGRAMAMOS PARA ELES. É COMO SE ELES TIVESSEM REGRAS PRÓPRIAS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img8.webp',
      textoImpresso:1,
      textoImpressoRecriado:'COMPUTADORES SEGUEM INSTRUÇÕES COM CONDIÇÕES QUE PROGRAMAMOS.',
      promptImagem:'pintura de um computador com diferentes símbolos de cheque e proibido flutuando',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'POR EXEMPLO, SE ESTIVERMOS JOGANDO UM JOGO E O PERSONAGEM CHEGAR A UMA PAREDE, PODEMOS PROGRAMAR O JOGO PARA QUE O PERSONAGEM MUDE DE DIREÇÃO E CONTINUE SE MOVENDO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/cond8.png',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'gere uma imagem em estilo pintura a óleo que mostre um cavaleiro visto de costas, olhando para uma grande parede',
      imagemImpresso:0,
      imagemFonte:'Cogna IA',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'A CONDICIONAL SERIA: <code><b>SE</b> O PERSONAGEM ENCONTRAR UMA PAREDE <br/>&nbsp;&nbsp;<b>ENTÃO</b> ELE DEVE MUDAR DE DIREÇÃO E CONTINUAR SE MOVENDO</code>',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/img9.webp',
      textoImpresso:1,
      textoImpressoRecriado:'EM UM JOGO, SE O PERSONAGEM CHEGAR NA PAREDE, O PROGRAMA FAZ ELE MUDAR DE DIREÇÃO E CONTINUAR SE MOVENDO.',
      promptImagem:'pintura de um jovem tocando uma parede com a mão',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'AS CONDICIONAIS PODEM SER USADAS DE MANEIRAS DIVERTIDAS E INTERESSANTES, NÃO É MESMO? ELAS NOS AJUDAM A TOMAR DECISÕES E A ENTENDER COMO AS COISAS FUNCIONAM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i9-condicionais/cond10.png',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'pintura de um jovem pensando em um parque',
      imagemImpresso:0,
      imagemFonte:'Cogna IA',
      textoDescritivoImagem:'',},
    ]}},

    licao16:{},

    licao17:{id:'CW1AULA17CODE',
    infografico:{titulo:'LIÇÃO 17. DETETIVES: APRENDENDO SOBRE DEPURAÇÃO',
    tituloSimplificado:'DEPURAÇÃO',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'HOJE VAMOS APRENDER SOBRE ALGO MUITO IMPORTANTE CHAMADO <b>DEPURAÇÃO</b>, OU <b>DEBUGGING</b>, EM INGLÊS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img1.jpeg',
      textoImpresso:0,
      textoImpressoRecriado:'HOJE VAMOS APRENDER SOBRE DEPURAÇÃO, OU DEBUG, EM INGLÊS.',
      promptImagem:'lupa de arte digital sobre um laptop',
      imagemImpresso:0,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'UM <b>BUG</b>, DO INGLÊS BICHO, É UM APELIDO PARA PROBLEMAS QUE ACONTECEM EM PROGRAMAS DE COMPUTADOR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img2.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'USAMOS BUG - "BICHO", EM INGLÊS - COMO APELIDO PARA ERROS EM PROGRAMAS.',
      promptImagem:'arte digital de um bug robótico 3D andando sobre um laptop em um fundo futurista e colorido',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'A DEPURAÇÃO - OU DEBUG, EM INGLÊS - É QUANDO PROCURAMOS OS ERROS, OU BUGS, EM PROGRAMAS. QUANDO INVESTIGAMOS ALGO, PROCURANDO POR ERROS, PODEMOS DIZER QUE FAZEMOS UMA DEPURAÇÃO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img7.webp',
      textoImpresso:1,
      textoImpressoRecriado:'DEBUG, OU DEPURAÇÃO, É O TRABALHO DE INVESTIGAÇÃO PARA DESCOBRIR ESSES ERROS.',
      promptImagem:'arte digital de um investigador robótico procurando bugs na tela de um computador em um fundo colorido',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'É MUITO DIFÍCIL FAZER UM PROGRAMA CERTO LOGO NA PRIMEIRA VEZ, É COMUM SURGIREM BUGS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img8.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'É MUITO DIFÍCIL FAZER UM PROGRAMA CERTO LOGO DA PRIMEIRA VEZ...',
      promptImagem:'arte digital de um computador com código flutuando em um fundo colorido',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'POR ISSO, OS PROGRAMADORES ESTÃO MUITO ACOSTUMADOS A USAR A DEPURAÇÃO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img10.webp',
      textoImpresso:1,
      textoImpressoRecriado:'POR ISSO OS PROGRAMADORES SÃO MUITO ACOSTUMADOS A USAR A DEPURAÇÃO.',
      promptImagem:'arte digital de depuração de computador',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'ELES TAMBÉM COSTUMAM FAZER ESSE PROCESSO EM PARES, ONDE UMA PESSOA PROGRAMA, ENQUANTO A OUTRA LÊ O CÓDIGO, PROCURANDO POR BUGS PARA CORRIGIR ANTES MESMO DE EXECUTAR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img11.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'APÓS CRIAR SEU PROGRAMA, ELES  PROCURAM POR BUGS E OS CORRIGEM, ANTES MESMO DE EXECUTAR.',
      promptImagem:'arte digital de programação em pares em um fundo colorido',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'A DEPURAÇÃO É UMA HABILIDADE IMPORTANTE QUE NOS AJUDA A RESOLVER PROBLEMAS E ENCONTRAR SOLUÇÕES.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i10-depuracao/img12.webp',
      textoImpresso:1,
      textoImpressoRecriado:'COM O HÁBITO DA DEPURAÇÃO, SEUS PROGRAMAS FUNCIONAM CORRETAMENTE E DE MODO MUITO MAIS RÁPIDO!',
      promptImagem:'arte digital de resolução de problemas em um fundo colorido',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},
    ]}},

    licao18:{},

    licao19:{},

    licao20:{id:'CW1AULA20CODE',
    infografico:{titulo:'LIÇÃO 20. OS JOGOS ROBÓTICOS DE INVERNO',
    tituloSimplificado:'JOGOS DE INVERNO',
    estiloPadrao:'basicoPaisagem',
      cenas:[
      {ordem:1,
      predecessor:0,
      texto:'HOJE VAMOS VOLTAR A LETRÓPOLIS, A CIDADE DOS ROBÔS QUE GOSTAM DE LETRAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i4-letropolis/img2.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'VAI COMEÇAR O MAIOR EVENTO DE LETRÓPOLIS!',
      promptImagem:'uma ilha nevada colorida e futurista',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'QUANDO CHEGAM AS FÉRIAS, A CIDADE TODA SOBE AS MONTANHAS PARA PRATICAR ESPORTES NA NEVE.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img3.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'QUANDO CHEGAM AS FÉRIAS, A CIDADE TODA SOBE AS MONTANHAS PARA PRATICAR ESPORTES NA NEVE.',
      promptImagem:'robô 3d engraçado esquiando em uma montanha nevada',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'COM UMA BOA PROGRAMAÇÃO, ROBÔS PODEM SER ÓTIMOS PILOTOS DE TRENÓ, PATINADORES DO GELO E ESQUIADORES.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/ji3.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'uma imagem colorida representando uma pista de gelo futurista com um robô 3D engraçado, colorido e bobo patinando nela',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'O SLALOM, QUE É UMA DESCIDA DE ESQUI COM OBSTÁCULOS, É O ESPORTE DE NEVE PREFERIDO DOS ROBÔS DE LETRÓPOLIS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'O ESPORTE DE NEVE PREFERIDO DOS ROBÔS DE LETRÓPOLIS É A “DESCIDA DE SKI COM OBSTÁCULOS”, TAMBÉM CHAMADA DE SLALOM.',
      promptImagem:'robô 3d engraçado esquiando em uma montanha nevada',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'NO SLALOM, A CADA OBSTÁCULO, OS ROBÔS DEVEM DESVIAR PARA UM LADO DIFERENTE, FAZENDO UM ZIGUE-ZAGUE.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img6.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'OS ROBÔS DEVEM DESVIAR DOS OBSTÁCULOS FAZENDO UM ZIGUEZAGUE, QUE EXIGE UMA EXCELENTE PROGRAMAÇÃO!',
      promptImagem:'robô 3d engraçado esquiando em uma montanha nevada, fazendo movimentos de zigue zague',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'COMO SABEMOS, OS ROBÔS NÃO FAZEM NADA SEM UMA BOA PROGRAMAÇÃO. É A CRIATIVIDADE DO PROGRAMADOR QUE OS FAZ ESQUIAR!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img8.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'VAI COMEÇAR A SUPER COMPETIÇÃO PARA SABER QUAL ROBÔ TEM A MELHOR PROGRAMAÇÃO PARA ESPORTES DA NEVE.',
      promptImagem:'robô 3d engraçado esquiando em uma montanha nevada, fazendo movimentos de zigue zague',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'E VOCÊS FORAM SELECIONADOS PARA PARTICIPAREM DAS EQUIPES DE PROGRAMAÇÃO DE SLALOM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img9.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'E VOCÊ FOI SELECIONADO PARA PARTICIPAR DA EQUIPE DE PROGRAMAÇÃO DE SLALOM!',
      promptImagem:'uma equipe de programadores programando um computador na neve',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'E AÍ? ESTÃO PRONTOS PARA PARTICIPAR DOS <b>JOGOS ROBÓTICOS DE INVERNO</b>? PONHAM SEUS AGASALHOS E MÃOS À OBRA!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i11-jogosinverno/img10.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'PRONTO PARA PARTICIPAR DOS JOGOS ROBÓTICOS DE INVERNO?',
      promptImagem:'robô 3d engraçado esquiando em uma montanha nevada, fazendo movimentos de zigue zague',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},
    ]}},
    
    licao21:{},
  
  },
  
  unidade4:{
	  titulo:'SENHAS E SEGURANÇA',
    descricao:'NESTA UNIDADE, VOCÊ APRENDERÁ UM POUCO SOBRE O USO DAS SENHAS PARA MANTER TUDO SEGURO NO SEU COMPUTADOR.',
    licao22:{id:'CW1AULA22CODE',
    infografico:{titulo:'LIÇÃO 22. UMA JORNADA SOBRE SENHAS E SEGURANÇA',
    tituloSimplificado:'SENHAS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'UMA SENHA É UMA SEQUÊNCIA DE LETRAS, NÚMEROS OU SÍMBOLOS QUE USAMOS PARA ACESSAR COISAS IMPORTANTES, COMO NOSSOS DISPOSITIVOS ELETRÔNICOS, CONTAS DE E-MAIL, JOGOS E MUITO MAIS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img4.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'O QUE É UMA SENHA?',
      promptImagem:'uma tela de computador mostrando uma caixa para digitar uma senha',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'A SENHA É COMO UMA CHAVE SECRETA QUE A GENTE CRIA E NÃO FALA PARA NINGUÉM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'',
      promptImagem:'Arte digital colorida de uma chave',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'ASSIM, SÓ NÓS CONSEGUIMOS ABRIR A CAIXA QUE GUARDA NOSSAS COISAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img10.webp',
      textoImpresso:1,
      textoImpressoRecriado:'',
      promptImagem:'Arte digital colorida de uma caixa com várias coisas saindo dela',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'UMA SENHA É CONSIDERADA "FORTE" OU BOA QUANDO É DIFÍCIL PARA OUTRAS PESSOAS ADIVINHAREM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img9.webp',
      textoImpresso:1,
      textoImpressoRecriado:'UMA SENHA BOA É DIFÍCIL DE ADIVINHAR.',
      promptImagem:'Arte digital colorida de um labirinto',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'QUANTO MAIS FORTE A SENHA, MAIS DIFÍCIL SERÁ PARA ALGUÉM DESCOBRÍ-LA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img11.webp',
      textoImpresso:0,
      textoImpressoRecriado:'QUANTO MAIS FORTE A SENHA, MAIS PROTEGIDO VOCÊ ESTÁ.',
      promptImagem:'Arte digital de personagem hackeando um computador',
      imagemImpresso:0,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'ASSIM COMO NÃO EMPRESTAMOS A CHAVE DE CASA PARA QUALQUER PESSOA, NÃO DEVEMOS COMPARTILHAR SENHAS COM NINGUÉM, ALÉM DE NOSSOS PAIS OU RESPONSÁVEIS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img13.webp',
      textoImpresso:1,
      textoImpressoRecriado:'SUA SENHA É SECRETA.',
      promptImagem:'Arte digital de um rosto com um dedo em frente à boca, fazendo sinal de segredoUma pintura digital vibrante do rosto de uma figura misteriosa com um dedo colorido pressionado contra os lábios, significando sigilo. O fundo é um caleidoscópio de cores rodopiantes, criando uma sensação de intriga e admiração.',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'ALÉM DAS SENHAS, É IMPORTANTE MANTER OS DISPOSITIVOS PROTEGIDOS. ISSO SIGNIFICA NÃO OS DEIXAR DESBLOQUEADOS OU SEM SUPERVISÃO PARA QUE OUTRAS PESSOAS NÃO POSSAM ACESSÁ-LOS SEM PERMISSÃO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img14.webp',
      textoImpresso:1,
      textoImpressoRecriado:'NÃO DEIXE O CELULAR OU TABLET DESBLOQUEADOS.',
      promptImagem:'crie uma imagem colorida de uma senhora guardiã de chaves, com muitas chaves no cinto e laptops, tablets e celulares bloqueados, flutuando',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'E SE RECEBERMOS MENSAGENS ESTRANHAS, PEDINDO SENHAS OU INFORMAÇÕES PESSOAIS, NUNCA DEVEMOS RESPONDER OU ABRIR.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/img15.webp',
      textoImpresso:1,
      textoImpressoRecriado:'NÃO ABRA MENSAGENS QUE NÃO CONHECE.',
      promptImagem:'crie uma imagem colorida de um laptop recebendo uma mensagem por um envelope e uma mão tentando recebê-la',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:9,
      predecessor:8,
      texto:'VAMOS APRENDER E MANTER NOSSOS DISPOSITIVOS E INFORMAÇÕES PROTEGIDOS JUNTOS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i12-senhas/ss9.png',
      textoImpresso:0,
      textoImpressoRecriado:'',
      promptImagem:'gere uma imagem colorida de várias crianças dando as mãos, tentando proteger um computador',
      imagemImpresso:0,
      imagemFonte:'Cogna IA',
      textoDescritivoImagem:'',},
    ]}},

    licao23:{},

    licao24:{},
   
    licao25:{id:'CW1AULA25CODE',
    infografico:{titulo:'LIÇÃO 25. A GRANDE DISPUTA: NUMERÓPOLIS VS. LETRÓPOLIS',
    tituloSimplificado:'NUMERÓPOLIS VS. LETRÓPOLIS',
    estiloPadrao:'basicoPaisagem',
      cenas:[
      {ordem:1,
      predecessor:0,
      texto:'VOCÊS SE LEMBRAM DE NUMERÓPOLIS, A CIDADE DOS ROBÔS QUE ADORAM NÚMEROS? E DE LETRÓPOLIS? A CIDADE QUE FICA NA ILHA GELADA, HABITADA POR ROBÔS QUE ADORAM LETRAS?',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/numxlet1.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'VOCÊ SE LEMBRA DAS CIDADES DE NUMERÓPOLIS E LETRÓPOLIS?',
      promptImagem:'cidade futurista colorida vista de cima/uma ilha nevada colorida e futurista',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'DEPOIS QUE AJUDAMOS AS DUAS CIDADES, OS ROBÔS FICARAM MUITO AMIGOS...',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img3.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'DEPOIS QUE VOCÊ AJUDOU AS CIDADES DE NUMERÓPOLIS E LETRÓPOLIS, OS ROBÔS DECIDIRAM...',
      promptImagem:'dois robôs engraçados apertando as mãos em uma rua futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'E CONCLUÍRAM QUE SERIA MELHOR PARA OS VISITANTES, QUE AMBAS AS CIDADES TIVESSEM O MESMO SISTEMA DE IDENTIFICAÇÃO DE RUAS:',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img4.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'... QUE SERIA MELHOR QUE AS DUAS CIDADES TIVESSEM O MESMO SISTEMA DE PLACAS NAS RUAS:',
      promptImagem:'uma rua colorida e futurista, com alguns sinais de trânsito vazios',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'OU AS DUAS USAM NÚMEROS; OU USAM LETRAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img5.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'OU AS DUAS USAM NÚMEROS; OU USAM LETRAS.',
      promptImagem:'sinais de trânsito em branco em uma rua futurista',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'MAS PARA NÃO DAR DISCUSSÃO - AFINAL, NINGUÉM ABRIA MÃO DE SUA PREFERÊNCIA - DECIDIRAM REALIZAR UMA GRANDE DISPUTA: “NUMERÓPOLIS VERSUS LETRÓPOLIS”.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img6.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'ENTÃO OS ROBÔS DECIDIRAM REALIZAR UMA GRANDE DISPUTA.',
      promptImagem:'dois robôs engraçados coloridos olhando um para o outro, em um octógono, enquanto eles começarão a luta',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'SE NUMERÓPOLIS VENCER, AS RUAS DAS DUAS CIDADES SERÃO NOMEADAS COM NÚMEROS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img7.webp',
      textoImpresso:1,
      textoImpressoRecriado:'SE NUMERÓPOLIS VENCER, AS RUAS DAS DUAS CIDADES TERÃO NÚMEROS...',
      promptImagem:'um close em uma placa de trânsito colorida e futurista em branco',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:7,
      predecessor:6,
      texto:'SE LETRÓPOLIS VENCER, COM LETRAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img8.webp',
      textoImpresso:1,
      textoImpressoRecriado:'SE LETRÓPOLIS VENCER, AS RUAS DAS DUAS CIDADES TERÃO LETRAS.',
      promptImagem:'um close em uma placa de trânsito colorida e futurista em branco',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'ROBÔS NÃO FAZEM NADA SEM UMA BOA PROGRAMAÇÃO, ENTÃO É PRECISO DEDICAÇÃO PARA DEFENDER UMA DAS CIDADES.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img9.webp',
      textoImpresso:1,
      textoImpressoRecriado:'VOCÊ FOI CONVOCADO PARA DEFENDER UMA DAS CIDADES.',
      promptImagem:'crie uma imagem colorida de um grupo de programadores defendendo uma cidade com programação',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',
      textoDescritivoImagem:'',},

      {ordem:9,
      predecessor:8,
      texto:'É A PROGRAMAÇÃO QUE IRÁ FAZER A DIFERENÇA NESTE DIVERTIDO DESAFIO.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img9.jpeg',
      textoImpresso:0,
      textoImpressoRecriado:'É A SUA PROGRAMAÇÃO QUE IRÁ FAZER A DIFERENÇA NESTE DESAFIO.',
      promptImagem:'um grupo de crianças em torno de um computador, programando-o',
      imagemImpresso:0,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:10,
      predecessor:9,
      texto:'QUEM VAI GANHAR? NUMERÓPOLIS? OU LETRÓPOLIS?',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i13-numerxletr/img10.png',
      textoImpresso:1,
      textoImpressoRecriado:'QUEM VAI GANHAR?',
      promptImagem:'',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},
    ]}},

    licao26:{},

    licao27:{id:'CW1AULA27CODE',
    infografico:{titulo:'LIÇÃO 27. A CIDADE DOS 1s e 0s: CONHECENDO BINARÓPOLIS',
    tituloSimplificado:'BINARÓPOLIS',
    estiloPadrao:'basicoPaisagem',
      cenas:[				
      {ordem:1,
      predecessor:0,
      texto:'VAMOS HOJE CONTINUAR EM NOSSA DIVERTIDA JORNADA, CONHECENDO A CIDADE MAIS ESTRANHA DA TERRA DOS ROBÔS: BINARÓPOLIS!',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img1.jpg',
      textoImpresso:0,
      textoImpressoRecriado:'HOJE VAMOS CONHECER BINARÓPOLIS.',
      promptImagem:'um país colorido e futurista, visto de cima, mostrando diferentes cidades',
      imagemImpresso:0,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:2,
      predecessor:1,
      texto:'EM BINARÓPOLIS OS ROBÔS TAMBÉM GOSTAM DE NÚMEROS, MAS SÓ CONHECEM OS NÚMEROS “ZERO” E “UM”. ESTRANHO NÃO?',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img3.webp',
      textoImpresso:1,
      textoImpressoRecriado:'EM BINARÓPOLIS, OS ROBÔS TAMBÉM AMAM OS NÚMEROS, MAS SÓ CONHECEM O UM E O ZERO.',
      promptImagem:'dois robôs engraçados conversando entre si com balões de fala em uma rua colorida e futurista',
      imagemImpresso:1,
      imagemFonte:'dall-e 3',      
      textoDescritivoImagem:'',},

      {ordem:3,
      predecessor:2,
      texto:'POIS É... COMO BINARÓPOLIS É UMA CIDADE MUITO ANTIGA, ELES ATÉ HOJE SÓ CONVERSAM ATRAVÉS DO CÓDIGO PRIMITIVO DOS COMPUTADORES, CONHECIDO COMO “SISTEMA BINÁRIO”.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img4.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'ESSE É O CÓDIGO PRIMITIVO DOS COMPUTADORES, TAMBÉM CONHECIDO COMO SISTEMA BINÁRIO.',
      promptImagem:'um robô 3D engraçado estudando um livro antigo em um ambiente colorido e futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:4,
      predecessor:3,
      texto:'E O “SISTEMA BINÁRIO” SÓ UTILIZA OS SÍMBOLOS ZERO E UM.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img5.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'E ESSE SISTEMA UTILIZA SOMENTE OS SÍMBOLOS 0 E 1.',
      promptImagem:'zeros e uns flutuando sobre um balão de fala em um cenário colorido e futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:5,
      predecessor:4,
      texto:'LÁ NO PROCESSADOR, O CÉREBRO DO COMPUTADOR, MINÚSCULOS COMPONENTES ELÉTRICOS SÃO LIGADOS PARA REPRESENTAR O “UM”, QUE DESENHAMOS COMO UMA CAIXINHA BRANCA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CS1/Binaropolis/img8.webp',
      textoImpresso:1,
      textoImpressoRecriado:'NO SISTEMA BINÁRIO, O UM É REPRESENTADO POR UMA CAIXINHA BRANCA, QUE QUER DIZER: LIGADO.',
      promptImagem:'um processador de computador com um cubo branco flutuando acima dele em um fundo colorido e futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:6,
      predecessor:5,
      texto:'E SÃO DESLIGADOS PARA REPRESENTAR O “ZERO”, QUE DESENHAMOS COMO UMA CAIXINHA PRETA.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CS1/Binaropolis/img9.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'E O ZERO É REPRESENTADO POR UMA CAIXINHA PRETA, QUE QUER DIZER: DESLIGADO.',
      promptImagem:'um processador de computador com um cubo preto flutuando acima dele em um fundo colorido e futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},      
      
      {ordem:7,
      predecessor:6,
      texto:'MAS SE BINARÓPOLIS TEM CINCO RUAS, COMO PODEREMOS DAR NOMES DIFERENTES PARA CADA RUA?',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img6.jpg',
      textoImpresso:1,
      textoImpressoRecriado:'MAS COMO PODEREMOS NOMEAR CINCO RUAS COM DOIS SÍMBOLOS?',
      promptImagem:'uma rua colorida, futurista e muito limpa',
      imagemImpresso:1,
      imagemFonte:'leonardo',
      textoDescritivoImagem:'',},

      {ordem:8,
      predecessor:7,
      texto:'O COMPUTADOR ENTÃO USA VÁRIOS “ZEROS” E “UNS”, UM AO LADO DO OUTRO, PARA REPRESENTAR DIFERENTES NÚMEROS E LETRAS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/cpsciimageinf/CS1/Binaropolis/img10.png',
      textoImpresso:1,
      textoImpressoRecriado:'PARA REPRESENTAR OS OUTROS NÚMEROS, USAMOS UMA SEQUÊNCIA DE CAIXINHAS BRANCAS E PRETAS.',
      promptImagem:'2 cubos pretos e 2 cubos brancos, formando uma linha de cubos num ambiente colorido e tecnológico',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},

      {ordem:9,
      predecessor:8,
      texto:'VAMOS APRENDER UM POUCO SOBRE ESSE CÓDIGO MISTERIOSO PARA AJUDAR BINARÓPOLIS.',
      urlAudioTexto:'',
      textoSimplificado:'',
      urlImagem:'https://storage.googleapis.com/mindmakers/Computational%20World/CW1/i14-binaropolis/img11.jpeg',
      textoImpresso:1,
      textoImpressoRecriado:'AGORA QUE VOCÊ CONHECE O SISTEMA BINÁRIO, VAI CONSEGUIR NUMERAR AS RUAS DE BINARÓPOLIS.',
      promptImagem:'dois robôs engraçados batendo os punhos em uma rua colorida e futurista',
      imagemImpresso:1,
      imagemFonte:'ideogram',
      textoDescritivoImagem:'',},
    ]}},
    
    licao28:{},
  
  },
  
  /*
	glossarioBilingue:{  }
  */

  capaVerso:{
	  urlImagemTerceiraCapa:'',
		textoTerceiraCapa:`OS AVIÕES AGORA SÃO MAIS SEGUROS E CONFORTÁVEIS PARA OS PASSAGEIROS GRAÇAS À TECNOLOGIA, QUE AJUDOU A MELHORAR VÁRIAS COISAS, NÃO SÓ NOS AVIÕES DE PASSAGEIROS, MAS NOS AVIÕES DE ENTREGAS TAMBÉM.
    ATUALMENTE, SERVIÇOS DE ENTREGAS PODEM SER FEITOS POR DRONES E OS PILOTOS DE AVIÕES DE PASASGEIROS USAM TABLETS NO LUGAR DE GUIAS ELETRÔNICOS, O QUE FAZ COM QUE O AVIÃO SEJA MAIS LEVE E ECONOMIZE COMBUSTÍVEL.
    EXISTEM, INCLUSIVE, EMPRESAS TRABALHANDO EM UMA FORMA DE RASTREAR OS AVIÕES EM TEMPO REAL USANDO REDES WI-FI. ISSO PODE SIGNIFICAR QUE EM BREVE TEREMOS APLICATIVOS NOS NOSSOS CELULARES QUE VÃO NOS MOSTRAR ONDE OS AVIÕES ESTÃO VOANDO.`,

		textoUltimaCapa: `O MUNDO COMPUTACIONAL NA AVIAÇÃO
    A COMPUTAÇÃO TORNOU A AVIAÇÃO MAIS SEGURA E EFICIENTE. GRAÇAS A SISTEMAS COMPUTADORIZADOS, AVIÕES SÃO MAIS PRECISOS EM VOAR E POUSAR. A SIMULAÇÃO DE VOO TREINA PILOTOS EM AMBIENTES SEGUROS, E O GERENCIAMENTO DE TRÁFEGO AÉREO AUTOMATIZADO MANTÉM OS CÉUS ORGANIZADOS.`
		},}